...
class InMemoryAdapterForEventSource : ForEventSource {
    ...
    override fun createStream(
        ...
        streams[streamName] = listOf(EventEntry(streamName = streamName, eventData = eventData, occurredOn = occurredOn))
        return "0"
    }

    override fun retrieveFromStream(streamName: String): List<RetrievedEvent> {
        return streams[streamName]!!.map { RetrievedEvent(event = it.eventData, occurredOn = it.occurredOn) }
    }
}

private data class EventEntry(val streamName: String, val eventData: ByteArray, val occurredOn: Instant) {
