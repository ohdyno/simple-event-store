Feature: Retrieve Events
  ...
  state of the stream is also returned. This version, in turn, can be used to append another event to the event stream. On the other hand,
  when retrieving from multiple streams, version not longer makes sense to return since it is only associated with a single stream.
  Instead, two other properties are returned:
  - the last event's id
